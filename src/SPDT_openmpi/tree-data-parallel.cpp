...
//     MPI_Comm_size(MPI_COMM_WORLD, &numtasks);  
//     if (taskid == 0)
//         printf("[%d]compress: begin\n", taskid);
//     MPI_Request reqs[numtasks];
//     MPI_Status status[numtasks];

//     float* buffer = new float[numtasks * SIZE];
...
//     }
//     if (taskid == 0)
//         printf("[%d]compress: begin communication\n", taskid);   
//     memcpy(&buffer[taskid * SIZE], histogram, sizeof(float) * SIZE);
//     Timer t;
//     t.reset();
//     for (int ii=0; ii < numtasks; ii++)
//         MPI_Ibcast(&buffer[taskid * SIZE], SIZE, MPI_FLOAT, taskid, MPI_COMM_WORLD, &reqs[taskid]);
//     MPI_Waitall(numtasks, reqs, status);
//     COMMUNICATION_TIME += t.elapsed();
//     if (taskid == 0)
//         printf("[%d]compress: end communication\n", taskid);   
//     if (taskid == 0){
//         float* histo = get_histogram_array(0, 0, 0);
//         print_array(histo);
//     }
...
void DecisionTree::compress(vector<Data> &data)
{
    ...
    //     float* histo1 = get_histogram_array(0, 5, 0);
    //     printf("[%d] -- ", taskid);
    //     print_array(histo1);
    // }
    ...
    if (taskid == MASTER){
        ...
        while(task_left > 0){
            ...
            for (int j=0; j<num_unlabled_leaves; j++){ // merge the results in the master thread
                for(int k=0; k<num_of_features; k++){
                    for(int c=0; c<num_of_classes; c++){
                        float* histo = get_histogram_array(j, k, c);
                    }
...
void get_gain(TreeNode* node, SplitPoint& split, int feature_id){
    ...
    if (taskid == MASTER){
        ...
        printf("px = %.7f, py_x0 = %.7f, py_x1 = %.7f\n", px, py_x0, py_x1);
    }
