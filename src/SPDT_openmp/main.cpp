...
int main(int argc, char **argv) {
    ...
    int max_num_leaf = -1;
    int max_depth = -1;
    while((c = getopt(argc, argv, "b:n:l:d:e")) != -1){
        switch (c)
        {
        case 'n':
    ...
    printf("num_of_thread=%d, max_num_leaf=%d, max_depth=%d, min_node_size=%d, max_bin_size=%d\n", 
            num_of_thread, max_num_leaf, max_depth, min_node_size, max_bin_size);
