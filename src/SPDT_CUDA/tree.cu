...
__global__ void
histogram_update_kernel(int data_size, int num_of_threads,
    Histogram *histogram_ptr, Data *data_ptr) {
...
void DecisionTree::compress(vector<Data> &data, vector<TreeNode *> &unlabeled_leaf) {
    int block_num = 0;
    int thread_per_block = this->datasetPointer->num_of_features;
    Histogram *cuda_histogram_ptr;
    Data *cuda_data_ptr;
    ...
    for (int i = 0; i < unlabeled_leaf.size(); i++) {
        ...
        block_num = node->data_ptr.size();
        
        cudaMalloc((void **)&cuda_histogram_ptr, 
            sizeof(Histogram) * this->datasetPointer->num_of_features * this->datasetPointer->num_of_classes);
        cudaMalloc((void **)&cuda_data_ptr,
        ...
        histogram_update_kernel<<<block_num, thread_per_block>>>(
            node->data_ptr.size(),
            this->datasetPointer->num_of_features,                        
            cuda_histogram_ptr,
            cuda_data_ptr);
